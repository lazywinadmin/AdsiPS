...
#>
	...
	PROCESS
	{
		TRY
		{
			...
			Write-Verbose -Message "[$FunctionName] [$ScriptName] Create DirectorySearcher"
			$Search = New-Object -TypeName System.DirectoryServices.DirectorySearcher -ErrorAction 'Stop'
			Write-Verbose -Message "[$FunctionName] [$ScriptName] Set SizeLimit of '$SizeLimit'"
			$Search.SizeLimit = $SizeLimit
			Write-Verbose -Message "[$FunctionName] [$ScriptName] Set Filter '(objectCategory=groupPolicyContainer)'"
			$Search.Filter = "(objectCategory=groupPolicyContainer)"
			
			IF ($PSBoundParameters['DomainDistinguishedName'])
			{
				Write-Verbose -Message "[$FunctionName] [$ScriptName] DomainDistinguishedName specified = '$DomainDistinguishedName'"
				IF ($DomainDistinguishedName -notlike "LDAP://*")
				{
					Write-Verbose -Message "[$FunctionName] [$ScriptName] DomainDistinguishedName notlike 'LDAP://*', prepending LDAP://"
			...
			{
				Write-Verbose -Message "[$FunctionName] [$ScriptName] Different Credential specified: '$($credential.username)'"
			...
			Write-Verbose -Message "[$FunctionName] [$ScriptName] Looking for objects..."
	...
	END
	{
		Write-Verbose -Message "[$FunctionName] [$ScriptName] End."
	}
}