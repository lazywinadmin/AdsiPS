...
function Move-ADSIDomainControllerRoles
{
      ...
      param ( [Parameter(Mandatory=$true)][string] $ComputerName = $null,
              [Management.Automation.PSCredential] $Credential = $null,
              [Parameter(Mandatory=$true)][ValidateSet("PdcRole","SchemaRole","NamingRole","RidRole","InfrastructureRole")][String[]] $Roles = $null,
              [Switch]$Force
              )
...
.SYNOPSIS
    ...
    Note that for now, only the UserPrincipalName can be changed. New versions of Set-ADSIUser will allow to modify additional attributes.
	
.PARAMETER UserPrincipalName
	Specify the UserPrincipalName 
	
.EXAMPLE
    Set-ADSIUser -SamAccountName micky -UserPrincipalName micky@ad.local
	
.NOTES
	Micky Balladelli
#>
	[CmdletBinding(SupportShouldProcess=$true,ConfirmImpact='Medium',DefaultParameterSetName = "SamAccountName")]
	PARAM (
	...
	PROCESS
	{
		TRY
		{
			...
			If ($PSBoundParameters['DisplayName'])
			{
				Write-Verbose -Message "[PROCESS] Parameter DisplayName: $DisplayName"
				$Search.filter = "(&(objectCategory=user)(name=$DisplayName))"
			}
			IF ($PSBoundParameters['GroupSamAccountName'])
			{
				Write-Verbose -Message "[PROCESS] Parameter SAMACCOUNTNAME: $SamAccountName"
				$Search.filter = "(&(objectCategory=user)(samaccountname=$SamAccountName))"
			}
			IF ($PSBoundParameters['GroupDistinguishedName'])
			{
				Write-Verbose -Message "[PROCESS] Parameter DISTINGUISHEDNAME: $DistinguishedName"
				$Search.filter = "(&(objectCategory=user)(distinguishedname=$DistinguishedName))"
			}
			...
			ELSE
			{
				IF ($Search.FindAll().Count -eq 0) { Write-Warning -Message "[PROCESS] No User Found" }
			}
		}#TRY
		CATCH
		{
